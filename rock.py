...
class cmdParser:
    ...
    def __init__(self, cmds):
        ...
        self.subparser = self.parser.add_subparsers(dest="cmd",help="All available cmmands")
        ...
        for i in cmds:
            cmd = cmds[i]
            sub_parser = self.subparser.add_parser(i, add_help=False, help=f"{i} command.")
...
class rock:
    ...
    def _crtCommands(self):
        bot_cmds = {"count": countd, "gif": gifCmd}
    ...
    def _fireRocket(self):
        self.rocket = RocketChatBot(conf.username, conf.password, server=conf.server, debug=conf.debug)
