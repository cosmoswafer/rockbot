...
### 2e. Error Handling — Compression Failure

If the compression LLM call fails (API error, timeout, empty response),
compression is **partially** executed: messages are **always pruned** from
Layer 1 (memory stripped to empty), but `summary.md` is **left unchanged**
(no placeholder written). An error reply is sent to the user informing them
that compression failed.

If the LLM returns text but the parser cannot find a summary section (e.g.
model put `## Used Knowledge` first), the full raw LLM output is used as the
summary. Only a completely empty LLM response (both `text` and
`reasoning_content` empty) triggers the error path.
...
| `error` | `Option<String>` | Error message if compression failed (summary.md left unchanged, but L1 pruned) |
...
A completely empty LLM response (both `text` and `reasoning_content` empty)
returns an error — `summary.md` is left unchanged and L1 messages are pruned.
...
| `summary` | `Option<String>` | The summary text generated by the LLM (if compression happened) |
| `was_explicit` | `bool` | Whether compression was triggered by `explicit_compress` flag |
| `error` | `Option<String>` | Error message if compression failed (summary.md unchanged, L1 pruned) |
