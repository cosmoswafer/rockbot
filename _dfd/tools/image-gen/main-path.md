...
`parameters()` schema are generated from the catalog at registry time
(issue #95) — see
...
flowchart TD
    ...
    PARSE -->|"merged with config defaults (quality, output_format, num_images, size_tier) + model alias resolved via ImageModelCatalog → (model_id, provider_name) → ImageGenParams.model_id + per-provider backend selected + uploaded image_urls + resolved image_size"| RESOLVE
    RESOLVE -->|"backend of the alias's provider (t2i or edit variant) + ImageGenParams (model_id set ⇒ per-call override; None ⇒ backend's configured default)"| PROVIDER
    PROVIDER --> GEN
